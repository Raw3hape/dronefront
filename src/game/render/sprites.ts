...
export async function loadAtlas(): Promise<Atlas> {
  ...
  const [dImgs, sImgs, exp, mis, maps] = await Promise.all([
    Promise.all(DRONE_KEYS.map((k) => loadImg(`/game/sprites/drones/${k}.png`))),
    Promise.all(SITE_KEYS.map((k) => loadImg(`/game/sprites/sites/${k}.png`))),
    Promise.all([0, 1, 2, 3].map((i) => loadImg(`/game/sprites/explode/e${i}.png`))),
    Promise.all([0, 1, 2, 3].map((i) => loadImg(`/game/sprites/missile/m${i}.png`))),
    Promise.all(MAP_KEYS.map((k) => loadImg(`/game/maps/${k}.jpg`))),
  ]);
