...
SCALE = 2
LOC_SRC = json.loads((ROOT / "src/game/catalog/loc.json").read_text())
LOC_LL = [tuple(p) for p in LOC_SRC["ll"]]
W, H = LOC_SRC["world"]
RW, RH = W * SCALE, H * SCALE

THEATERS = {
    "front": {
        "bbox": tuple(LOC_SRC["bbox"]["front"]),
        "title": "UKRAINE  —  RUSSIA",
        "ua": (31.2, 48.55),
        "ru": (39.6, 50.9),
    },
    "north": {
        "bbox": tuple(LOC_SRC["bbox"]["north"]),
        "title": "KHARKIV  —  BELGOROD",
        "ua": (34.35, 49.35),
        "ru": (38.15, 51.55),
    },
    "south": {
        "bbox": tuple(LOC_SRC["bbox"]["south"]),
